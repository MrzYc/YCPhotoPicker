...
@interface YCImageSelectController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate>
{
...
@property (nonatomic, strong) YCCollectionView *collectionView;
...
@implementation YCImageSelectController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (iOS9Later) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[YCPhotoPickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[YCPhotoPickerController class], nil];
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && iOS8Later && self->_model.isCameraRoll) {
            [[YCImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:YES completion:^(YCAlbumModel *model) {
                self->_model = model;
            ...
            if (self->_showTakePhotoBtn || !iOS8Later || self->_isFirstAppear) {
                [[YCImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<YCAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    _collectionView = [[YCCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
    ...
    if (_showTakePhotoBtn) {
        _collectionView.contentSize = CGSizeMake(self.view.yc_width, ((_model.count + self.columnNumber) / self.columnNumber) * self.view.yc_width);
    } else {
        _collectionView.contentSize = CGSizeMake(self.view.yc_width, ((_model.count + self.columnNumber - 1) / self.columnNumber) * self.view.yc_width);
    }
    [self.view addSubview:_collectionView];
    [_collectionView registerClass:[YCAssetCell class] forCellWithReuseIdentifier:@"YCAssetCell"];
    [_collectionView registerClass:[YCAssetCameraCell class] forCellWithReuseIdentifier:@"YCAssetCameraCell"];
}
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [YCCommonTools yc_isIPhoneX] ? 50 + (83 - 49) : 50;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (iOS7Later && !isStatusBarHidden) top += [YCCommonTools yc_statusBarHeight];
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.yc_height - toolBarHeight - top : self.view.yc_height - top;;
    } else {
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.yc_height - toolBarHeight : self.view.yc_height;
    }
    _collectionView.frame = CGRectMake(0, top, self.view.yc_width, collectionViewHeight);
    CGFloat itemWH = (self.view.yc_width - (self.columnNumber + 1) * itemMargin) / self.columnNumber;
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        toolBarTop = self.view.yc_height - toolBarHeight;
    } else {
        CGFloat navigationHeight = naviBarHeight;
        if (iOS7Later) navigationHeight += [YCCommonTools yc_statusBarHeight];
        toolBarTop = self.view.yc_height - toolBarHeight - navigationHeight;
    }
    _bottomToolBar.frame = CGRectMake(0, toolBarTop, self.view.yc_width, toolBarHeight);
    CGFloat previewWidth = [tzImagePickerVc.previewBtnTitleStr yc_calculateSizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} maxSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)].width + 2;
    if (!tzImagePickerVc.allowPreview) {
        previewWidth = 0.0;
    }
    _previewButton.frame = CGRectMake(10, 3, previewWidth, 44);
    _previewButton.yc_width = !tzImagePickerVc.showSelectBtn ? 0 : previewWidth;
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat fullImageWidth = [tzImagePickerVc.fullImageBtnTitleStr yc_calculateSizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} maxSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)].width;
        _originalPhotoButton.frame = CGRectMake(CGRectGetMaxX(_previewButton.frame), 0, fullImageWidth + 56, 50);
        _originalPhotoLabel.frame = CGRectMake(fullImageWidth + 46, 0, 80, 50);
    }
    [_doneButton sizeToFit];
    _doneButton.frame = CGRectMake(self.view.yc_width - _doneButton.yc_width - 12, 0, _doneButton.yc_width, 44);
    _numberImageView.frame = CGRectMake(_doneButton.yc_left - 30 - 2, 7, 30, 30);
    _numberLabel.frame = _numberImageView.frame;
    _divideLine.frame = CGRectMake(0, 0, self.view.yc_width, 1);
    
    [YCImageManager manager].columnNumber = [YCImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    YCPhotoPreviewController *photoPreviewVc = [[YCPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.minImagesCount && tzImagePickerVc.selectedModels.count < tzImagePickerVc.minImagesCount) {
        NSString *title = [NSString stringWithFormat:[NSBundle localizedStringForKey:@"Select a minimum of %zd photos"], tzImagePickerVc.minImagesCount];
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            YCAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        [YCImageManager manager].shouldFixOrientation = YES;
        ...
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ...
            [[YCImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                ...
                if (photo) {
                    photo = [[YCImageManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                ...
                if (progress < 1 && havenotShowAlert && !alertView) {
                    [tzImagePickerVc hideProgressHUD];
                    alertView = [tzImagePickerVc showAlertWithTitle:[NSBundle localizedStringForKey:@"Synchronizing photos from iCloud"]];
                    havenotShowAlert = NO;
                    return;
                }
                if (progress >= 1) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.row >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.row == 0)) && _showTakePhotoBtn) {
        YCAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"YCAssetCameraCell" forIndexPath:indexPath];
    ...
    YCAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"YCAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        YCPhotoPickerController *tzImagePickerVc = (YCPhotoPickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (YCAssetModel *model_item in selectedModels) {
                if ([[[YCImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[YCImageManager manager] getAssetIdentifier:model_item.asset]]) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                NSString *title = [NSString stringWithFormat:[NSBundle localizedStringForKey:@"Select a maximum of %zd photos"], tzImagePickerVc.maxImagesCount];
                [tzImagePickerVc showAlertWithTitle:title];
            }
        }
        [UIView showOscillatoryAnimationWithLayer:strongLayer type:YCOscillatoryAnimationToSmaller];
    };
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    YCAssetModel *model = _models[index];
    if (model.type == YCAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            YCPhotoPickerController *imagePickerVc = (YCPhotoPickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            YCVideoPlayerController *videoPlayerVc = [[YCVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == YCAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            YCPhotoPickerController *imagePickerVc = (YCPhotoPickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            YCGifPhotoPreviewController *gifPreviewVc = [[YCGifPhotoPreviewController alloc] init];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied) && iOS7Later) {
        ...
        if (iOS8Later) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle localizedStringForKey:@"Can not use camera"] message:message delegate:self cancelButtonTitle:[NSBundle localizedStringForKey:@"OK"] otherButtonTitles:nil];
            [alert show];
        }
...
- (void)pushPhotoPrevireViewController:(YCPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[YCImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.yc_width <= 320) {
        return;
    }
    YCPhotoPickerController *imagePickerVc = (YCPhotoPickerController *)self.navigationController;
    [[YCImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    ...
    YCPhotoPickerController *tzImagePickerVc = (YCPhotoPickerController *)self.navigationController;
    for (YCAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[YCImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
        ...
        if (videoUrl) {
            [[YCImageManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(NSError *error) {
                if (!error) {
                    [self reloadPhotoArray];
                }
            }];
...
- (void)reloadPhotoArray {
    YCPhotoPickerController *tzImagePickerVc = (YCPhotoPickerController *)self.navigationController;
    [[YCImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:NO completion:^(YCAlbumModel *model) {
        self->_model = model;
        [[YCImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<YCAssetModel *> *models) {
            [tzImagePickerVc hideProgressHUD];
            
            YCAssetModel *assetModel;
            if (tzImagePickerVc.sortAscendingByModificationDate) {
...
- (void)resetCachedAssets {
    [[YCImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[YCImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[YCImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            YCAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
