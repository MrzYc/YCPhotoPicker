...
@implementation YCAssetCell

- (void)setModel:(YCAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[YCImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[YCImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.yc_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[YCImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)setType:(YCAssetCellType)type {
    _type = type;
    if (type == YCAssetCellTypePhoto || type == YCAssetCellTypeLivePhoto || (type == YCAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == YCAssetCellTypeVideo) {
        self.bottomView.hidden = NO;
        self.timeLength.text = _model.timeLength;
        self.videoImgView.hidden = NO;
        _timeLength.yc_left = self.videoImgView.yc_right;
        _timeLength.textAlignment = NSTextAlignmentRight;
    } else if (type == YCAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
        self.timeLength.text = @"GIF";
        self.videoImgView.hidden = YES;
        _timeLength.yc_left = 5;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:YCOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self fetchBigImage];
    }else {  // 取消选中，取消大图的获取
...
- (void)fetchBigImage {
    _bigImageRequestID = [[YCImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (self->_progressView) {
            [self hideProgressView];
        }
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.yc_width - 44, 0, 44, 44);
    } else {
        _selectPhotoButton.frame = self.bounds;
    }
    _selectImageView.frame = CGRectMake(self.yc_width - 27, 0, 27, 27);
    _imageView.frame = CGRectMake(0, 0, self.yc_width, self.yc_height);
    
    static CGFloat progressWH = 20;
    CGFloat progressXY = (self.yc_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);
    
    _bottomView.frame = CGRectMake(0, self.yc_height - 17, self.yc_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.yc_right, 0, self.yc_width - self.videoImgView.yc_right - 5, 17);
...
@interface YCAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end


@implementation YCAlbumCell
...
- (void)setModel:(YCAlbumModel *)model {
    _model = model;
    ...
    [[YCImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    ...
    _selectedCountButton.frame = CGRectMake(self.yc_width - 24 - 30, 23, 24, 24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80, (self.yc_height - titleHeight) / 2, self.yc_width - 80 - 50, titleHeight);
...
@implementation YCAssetCameraCell
