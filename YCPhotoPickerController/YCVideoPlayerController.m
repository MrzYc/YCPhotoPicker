...
@interface YCVideoPlayerController ()
{
    AVPlayer *_player;
...
@implementation YCVideoPlayerController
...
- (void)viewDidLoad {
    ...
    YCPhotoPickerController *ycImagePickerVc = (YCPhotoPickerController *)self.navigationController;
    if (ycImagePickerVc) {
        self.navigationItem.title = ycImagePickerVc.previewBtnTitleStr;
    }
...
- (void)configMoviePlayer  {
    [[YCImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (!isDegraded && photo) {
            self->_cover = photo;
    ...
    [[YCImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
/// Show progress，do it next time / 给播放器添加进度更新
...
- (void)configBottomToolBar {
    ...
    YCPhotoPickerController *ycImagePickerVc = (YCPhotoPickerController *)self.navigationController;
    if (ycImagePickerVc) {
        [_doneButton setTitle:ycImagePickerVc.doneBtnTitleStr forState:UIControlStateNormal];
        [_doneButton setTitleColor:ycImagePickerVc.oKButtonTitleColorNormal forState:UIControlStateNormal];
    } else {
        [_doneButton setTitle:[NSBundle localizedStringForKey:@"Done"] forState:UIControlStateNormal];
        [_doneButton setTitleColor:[UIColor colorWithRed:(83/255.0) green:(179/255.0) blue:(17/255.0) alpha:1.0] forState:UIControlStateNormal];
    }
    [_doneButton setTitleColor:ycImagePickerVc.oKButtonTitleColorDisabled forState:UIControlStateDisabled];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarAndNaviBarHeight = statusBarHeight + self.navigationController.navigationBar.yc_height;
    ...
    _playButton.frame = CGRectMake(0, statusBarAndNaviBarHeight, self.view.yc_width, self.view.yc_height - statusBarAndNaviBarHeight - toolBarHeight);
}
