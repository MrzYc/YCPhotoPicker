...
@implementation YCImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.yc_width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[YCImageManager manager] fixOrientation:cropedImage];
...
+ (UIImage *)sd_yc_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [YCImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 200;
