...
@interface YCPhotoPreviewController () <UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate>{
...
@implementation YCPhotoPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [YCImageManager manager].shouldFixOrientation = YES;
    __weak typeof(self) weakSelf = self;
    YCPhotoPickerController *_tzImagePickerVc = (YCPhotoPickerController *)weakSelf.navigationController;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_currentIndex) [_collectionView setContentOffset:CGPointMake((self.view.yc_width + 20) * _currentIndex, 0) animated:NO];
...
- (void)configBottomToolBar {
    ...
    YCPhotoPickerController *_tzImagePickerVc = (YCPhotoPickerController *)self.navigationController;
...
- (void)configCollectionView {
    ...
    _collectionView.contentSize = CGSizeMake(self.models.count * (self.view.yc_width + 20), 0);
    [self.view addSubview:_collectionView];
    [_collectionView registerClass:[YCPhotoPreviewCell class] forCellWithReuseIdentifier:@"YCPhotoPreviewCell"];
    [_collectionView registerClass:[YCVideoPreviewCell class] forCellWithReuseIdentifier:@"YCVideoPreviewCell"];
    [_collectionView registerClass:[YCGifPreviewCell class] forCellWithReuseIdentifier:@"YCGifPreviewCell"];
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop) {
        ...
        [YCImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = [YCCommonTools yc_statusBarHeight];
    CGFloat statusBarHeightInterval = statusBarHeight - 20;
    CGFloat naviBarHeight = statusBarHeight + _tzImagePickerVc.navigationBar.yc_height;
    _naviBar.frame = CGRectMake(0, 0, self.view.yc_width, naviBarHeight);
    _backButton.frame = CGRectMake(10, 10 + statusBarHeightInterval, 44, 44);
    _selectButton.frame = CGRectMake(self.view.yc_width - 54, 10 + statusBarHeightInterval, 42, 42);
    
    _layout.itemSize = CGSizeMake(self.view.yc_width + 20, self.view.yc_height);
    _layout.minimumInteritemSpacing = 0;
    _layout.minimumLineSpacing = 0;
    _collectionView.frame = CGRectMake(-10, 0, self.view.yc_width + 20, self.view.yc_height);
    ...
    CGFloat toolBarTop = self.view.yc_height - toolBarHeight;
    _toolBar.frame = CGRectMake(0, toolBarTop, self.view.yc_width, toolBarHeight);
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat fullImageWidth = [_tzImagePickerVc.fullImageBtnTitleStr yc_calculateSizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} maxSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)].width;
...
- (void)select:(UIButton *)selectButton {
    YCPhotoPickerController *_tzImagePickerVc = (YCPhotoPickerController *)self.navigationController;
    YCAssetModel *model = _models[_currentIndex];
    if (!selectButton.isSelected) {
        // 1. select:check if over the maxImagesCount / 选择照片,检查是否超过了最大个数的限制
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:[NSBundle localizedStringForKey:@"Select a maximum of %zd photos"], _tzImagePickerVc.maxImagesCount];
            ...
            if (model.type == YCAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                [_tzImagePickerVc showAlertWithTitle:[NSBundle localizedStringForKey:@"Select the video when in multi state, we will handle the video as a photo"]];
            }
        }
    } else {
        ...
        for (YCAssetModel *model_item in selectedModels) {
            if ([[[YCImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[YCImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    YCAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:YCOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:YCOscillatoryAnimationToSmaller];
}
...
- (void)doneButtonClick {
    ...
    if (_progress > 0 && _progress < 1 && (_selectButton.isSelected || !_tzImagePickerVc.selectedModels.count )) {
        _alertView = [_tzImagePickerVc showAlertWithTitle:[NSBundle localizedStringForKey:@"Synchronizing photos from iCloud"]];
        return;
    }
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        YCPhotoPreviewCell *cell = (YCPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [YCImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [YCImageCropManager circularClipImage:cropedImage];
        }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    offSetWidth = offSetWidth +  ((self.view.yc_width + 20) * 0.5);
    
    NSInteger currentIndex = offSetWidth / (self.view.yc_width + 20);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    YCPhotoPickerController *_tzImagePickerVc = (YCPhotoPickerController *)self.navigationController;
    YCAssetModel *model = _models[indexPath.row];
    
    YCAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == YCAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"YCVideoPreviewCell" forIndexPath:indexPath];
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == YCAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"YCGifPreviewCell" forIndexPath:indexPath];
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"YCPhotoPreviewCell" forIndexPath:indexPath];
        YCPhotoPreviewCell *photoPreviewCell = (YCPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[YCPhotoPreviewCell class]]) {
        [(YCPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[YCPhotoPreviewCell class]]) {
        [(YCPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[YCVideoPreviewCell class]]) {
        [(YCVideoPreviewCell *)cell pausePlayerAndShowNaviBar];
    }
}
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    // 如果正在预览的是视频，隐藏原图按钮
    if (!_isHideNaviBar) {
        if (model.type == YCAssetModelMediaTypeVideo) {
    ...
    if (![[YCImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)showPhotoBytes {
    [[YCImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
